    ...
    if (getSession().editorRef != null) {
      ...
      getSession().editorRef.onDidChangeModelContent((event) => {
        event.changes.forEach(change => {
          ...
          const actions = { Action: action, Start_Line: startLine, End_Line: endLine, Start_Column: startColumn, End_Column: endColumn, Text: text, Timestamp: new Date().toISOString() }
